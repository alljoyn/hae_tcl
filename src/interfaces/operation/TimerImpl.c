...
AJ_Status TimerInterfaceOnMethodHandler(AJ_Message* msg, const char* objPath, uint8_t memberIndex, void* listener)
{
    AJ_Status status = AJ_OK;
    ...
    case 7 :
        if (!lt->OnSetTargetTimeToStart) {
            ...
            if (status == AJ_OK) {
                status = lt->OnSetTargetTimeToStart(objPath, targetTimeToStart);
            }
        }
        break;
    case 8 :
        if (!lt->OnSetTargetTimeToStop) {
            ...
            if (status == AJ_OK) {
                status = lt->OnSetTargetTimeToStop(objPath, targetTimeToStop);
            }
        }
        break;
    default:
