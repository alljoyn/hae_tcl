...
AJ_Status HeatingZoneInterfaceOnGetProperty(AJ_Message* replyMsg, const char* objPath, void* properties, uint8_t memberIndex, void* listener)
{
    ...
    case 2 :
        {
            uint8_t* maxHeatingLevels = (uint8_t*)malloc(sizeof(uint8_t) * listSize);
            if (lt && lt->OnGetMaxHeatingLevels) {
                ...
                if (status == AJ_OK) {
                    ...
                    props->maxHeatingLevels = (uint8_t*)malloc(sizeof(uint8_t) * listSize);

                    memcpy(props->maxHeatingLevels, maxHeatingLevels, sizeof(uint8_t) * listSize);
                }
            }
            if (!props->maxHeatingLevels) {
            ...
            status = AJ_MarshalArgs(replyMsg, "ay", props->maxHeatingLevels, sizeof(uint8_t) * listSize);
            free(maxHeatingLevels);
        }
        break;
    case 3:
...
AJ_Status Hae_HeatingZoneInterfaceGetMaxHeatingLevels(const char* objPath, uint8_t* maxHeatingLevels)
{
    ...
    if (props) {
        ...
        memcpy(maxHeatingLevels, props->maxHeatingLevels, sizeof(uint8_t) * listSize);
    } else {
        status = AJ_ERR_NO_MATCH;
    }
