...
AJ_Status CycleControlInterfaceOnMethodHandler(AJ_Message* msg, const char* objPath, uint8_t memberIndex, void* listener)
{
    AJ_Status status = AJ_OK;
    ...
    case 4 :
        if (!lt->OnExecuteOperationalCommand) {
            ...
            if (status == AJ_OK) {
                status = lt->OnExecuteOperationalCommand(objPath, command);
            }
        }
        break;
    default:
