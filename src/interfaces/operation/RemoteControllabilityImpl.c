...
AJ_Status RemoteControllabilityInterfaceEmitPropertiesChanged(AJ_BusAttachment* busAttachment, const char* objPath, void* properties, uint8_t memberIndex)
{
    AJ_Status status = AJ_OK;
...
AJ_Status Hae_RemoteControllabilityInterfaceSetIsControllable(AJ_BusAttachment* busAttachment, const char* objPath, bool* isControllable)
{
    AJ_Status status = AJ_OK;
    RemoteControllabilityProperties* props = NULL;
    ...
    props = (RemoteControllabilityProperties*)GetProperties(objPath, REMOTE_CONTROLLABILITY_INTERFACE);
    if (props) {
        if (props->isControllable != *isControllable) {
            props->isControllable = *isControllable;
