        ...
        if (propertyStoreProperties[fieldIndex].mode7Public && (filter.bit0About || (filter.bit2Announce && propertyStoreProperties[fieldIndex].mode1Announce))) {
            ...
            if (value == NULL && (int8_t)fieldIndex >= (int8_t)AJSVC_PROPERTY_STORE_NUMBER_OF_MANDATORY_KEYS) {     // Non existing values are skipped!
                ...
                if (fieldIndex == AJSVC_PROPERTY_STORE_APP_ID) {
                    ...
                    status = AJ_MarshalVariant(msg, "a(uo)");
                    if (status != AJ_OK) {
                        return status;
                    }

                    status = AJ_MarshalContainer(msg, &arr, AJ_ARG_ARRAY);
                    if (status != AJ_OK) {
                        return status;
                    }
                    ...
                    for (i = 0; i < count; i++)
                    {
                        ...
                        status = AJ_MarshalArgs(msg, "uo", deviceTypeDescription[i].type, deviceTypeDescription[i].objectpath );
                        if (status != AJ_OK) {
                            return status;
                        }
                    ...
                    status = AJ_MarshalCloseContainer(msg, &arr);
                    if (status != AJ_OK) {
                        return status;
                    }
