...
AJ_Status OnSetRepeatMode(const char* objPath, bool repeatMode)
{
...
AJ_Status OnSetResourceSavingMode(const char* objPath, bool resourceSavingMode)
{
...
AJ_Status OnGetSupportedCyclePhases(const char* objPath, uint8_t* supportedCyclePhases)
{
    ...
    for (i = 0 ; i < 2; i++) {
        supportedCyclePhases[i] = (uint16_t) i;
    }
...
AJ_Status InitHaeProperties(AJ_BusAttachment* busAttachment)
{
    ...
    uint8_t levelRead = 0;
    uint8_t supportedCyclePhases[2] = { 0, 1 };
    uint8_t supportedCyclePhasesRead[2];
    ...
    status = Hae_RobotCleaningCyclePhaseInterfaceSetSupportedCyclePhases(busAttachment, HAE_OBJECT_PATH_CONTROLLEE, supportedCyclePhases, 2);
