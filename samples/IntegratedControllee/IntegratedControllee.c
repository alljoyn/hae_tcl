...
{
    printf("OnSetTargetLevel : %s, targetLevel : %u\n", objPath, targetLevel);
...
AJ_Status InitHaeProperties(AJ_BusAttachment* busAttachment)
{
    ...
    /* WaterLevel IF */
    uint8_t supplySourceRead  = 0;
    uint8_t currentLevelRead  = 5;
    uint8_t maxLevelRead = 10;

    /* SoilLevel IF */
    uint8_t maxSoilLevelRead = 5;
    uint8_t targetLevel = 0;
    uint8_t targetLevelRead = 0;
    const uint8_t selectableLevels[3] = { 0, 1, 2 };
    ...
    status = Hae_WaterLevelInterfaceSetMaxLevel(busAttachment, HAE_OBJECT_PATH_CONTROLLEE, maxLevelRead);
...
int AJ_Main(void)
{
    ...
    soilLevelListener.OnSetTargetLevel = OnSetTargetLevel;
