...
AJ_Status InitHaeDishWashingProperties(AJ_BusAttachment* busAttachment)
{
    #define SUPPORTED_CYCLEPHASE_NUM   5

    AJ_Status status = AJ_OK;
    int i = 0;

    uint8_t cyclePhase = 0;
    const uint8_t supportedCyclePhases[SUPPORTED_CYCLEPHASE_NUM] = {0x00, 0x01, 0x02, 0x03, 0x04};

    uint8_t cyclePhaseRead;
    uint8_t supportedCyclePhasesRead[SUPPORTED_CYCLEPHASE_NUM];

    /* write in this order (cycle phase as last) to pass the validation */
    if (status == AJ_OK) {
        status = Hae_DishWashingCyclePhaseInterfaceSetSupportedCyclePhases(busAttachment, HAE_OBJECT_PATH_CONTROLLEE, supportedCyclePhases, SUPPORTED_CYCLEPHASE_NUM);
    }
    if (status == AJ_OK) {
...
AJ_Status InitHaeProperties(AJ_BusAttachment* busAttachment)
{
    ...
    if (status == AJ_OK) {
        status = InitHaeDishWashingProperties(busAttachment);
    }
